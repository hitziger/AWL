...
for i=1:nrows
    for j=1:ntrials
        subaxis(nrows,ntrials,(i-1)*ntrials+j,'Spacing', 0.02, 'MarginBottom', 0.07, 'MarginLeft', 0.05, 'MarginRight', 0.01, 'MarginTop', 0.06)  
        plot(t,data{i}(:,j),'k')
        if i==1
...
for i=1:nrows
    for j=1:ncols
        ...
        if i==1
            title(['original waveform ' num2str(j)],'fontsize',14)
        end
        if i==nrows
            xlabel('time [s]','fontsize',14)
        else
...
for i=1:nrows
    for j=1:ncols
        subaxis(nrows,ncols,(i-1)*ncols+j,'Spacing', 0.02, 'MarginBottom', 0.07, 'MarginLeft', 0.05, 'MarginRight', 0.01, 'MarginTop', 0.06)
        plot(t,data{i}(:,j)/norm(data{i}(:,j)),'k')
        if i==1
            title(['waveform ' num2str(j)],'fontsize',14)
        end
        if i==nrows
            xlabel('time [s]','fontsize',14)
        else
