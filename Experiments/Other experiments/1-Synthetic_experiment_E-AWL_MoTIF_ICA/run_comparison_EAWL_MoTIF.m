...
tborder = 10;            % maximal shift in seconds (time window is [0 5])
nsignals = 200;         % number of signals

% parameters for processing
ncases = 4;                 % see the four settings described above
nsimulations = 25;          % number of simulations per case 1)-4)
...
sigma_lat_range = [-3,0.699];   % range of lat SDs (log10), for 2)
...
[D_ext,t_temp,t,next] = generate_dictionary(fsamp, tborder);
...
% get t_ext for extended trials 
[~,t_ext,~,next] = generate_dictionary(fsamp, 5);

%%%%% SET UP STUFF FOR LEARNING %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nmethods = 4;
kernel_error = zeros(nmethods,ncases,nsimulations);
pink_noise = zeros(length(t),nsignals);
pink_noise_ext = zeros(length(t_ext),nsignals);
for i=1:nsignals
    pink_noise(:,i) = pinknoise(length(t));
    pink_noise_ext(:,i) = pinknoise(length(t_ext));
end
parTrials = struct;
...
for Kvar = 1:K
    ...
    [X,~,~] = create_examples(D_ext(:,1:Kvar),t,parTrials);
    ...
    method_id = 1;
    % zeropad trials 
    l_atoms = size(X_train,1);
    X_long = [zeros(nlatencies,nsignals); X_train; zeros(nlatencies,nsignals)];
    [~,D_motif,A_motif] = MoTIF_learning(X_long,l_atoms,Kvar,1,0);
    % normalize MoTIF atoms
    for k=1:Kvar
...
for i = 1:length(sigma_amps)
    ...
    parTrials.sigmaT = 0;
    [X,~,~] = create_examples(D_ext,t,parTrials);
    sigma_X = sqrt(var(reshape(X,[1 size(X,1)*size(X,2)])));
    sigma_noise = sigma_X/10^(SNR_def/20);
    X_train = X + sigma_noise*pink_noise;
    norm_X = sqrt(sum(sum(X.^2)));    
    ...
    l_atoms = size(X_train,1);
    X_long = [zeros(nlatencies,nsignals); X_train; zeros(nlatencies,nsignals)];
    [~,D_motif,A_motif] = MoTIF_learning(X_long,l_atoms,K,1,0);
    % normalize MoTIF atoms
    for k=1:K
        norm_k = norm(D_motif(:,k));
        D_motif(:,k) = D_motif(:,k) / norm_k;
...
for i = 1:length(sigma_lats)
    ...
    [X_ext,~,~] = create_examples(D_ext,t_ext,parTrials);
    sigma_X = sqrt(var(reshape(X_ext,[1 size(X_ext,1)*size(X_ext,2)])));
    sigma_noise = sigma_X/10^(SNR_def/20);
    X_train = X_ext + sigma_noise*pink_noise_ext;
    norm_X = sqrt(sum(sum(X.^2)));
    
    % MoTIF
    method_id = 1;
    l_atoms = length(t);
    [~,D_motif,A_motif] = MoTIF_learning(X_train,l_atoms,K,1,0);
    % normalize MoTIF atoms
    for k=1:K
        norm_k = norm(D_motif(:,k));
        D_motif(:,k) = D_motif(:,k) / norm_k;
    ...
    % ICA     
    method_id = method_id+1;
    [D_ica_long, A_ica, ~] = fastica(X_train','lastEig',K,'numOfIC',K,'stabilization','on');
    A_ica = A_ica';         
    D_ica_long = D_ica_long';     
    [D_ica_long,A_ica] = sign_correction_dict(D_ica_long,A_ica);
    % cut to right size
    D_ica = D_ica_long(next+1:next+l_atoms,:);
    for k=1:K
        normk = norm(D_ica(:,k));
        D_ica(:,k) = D_ica(:,k)/normk;
        A_ica(k,:) = A_ica(k,:) * normk;
    ...
    for k=1:K              
        display(['learning ' num2str(k) ' atoms'])
        new = randn(size(X_train,1)+2*nlatencies,1);
        parAWL.D=[D_awl,new];
        [D_awl,code,~]=mexEAWL(X_train,parAWL); 
    end
    %Xrec = compose_signals(D_awl,code.A,code.Delta,nlatencies);
    %res = X_train - Xrec;
    D_awl = D_awl(nlatencies+next+1:end-nlatencies-next,:);
    for k=1:K
        D_awl(:,k)=D_awl(:,k)/norm(D_awl(:,k));
    end
    kernel_error(method_id,case_id,i) = kernel_dist_shift(D,D_awl);
    ...
    % AWL-I
    method_id = method_id+1;
    D_awli = [zeros(nlatencies,K); D_ica_long; zeros(nlatencies,K)];          
    display(['learning ' num2str(K) ' atoms, ICA init'])
    parAWL.D=D_awli;
    [D_awli,code,~]=mexEAWL(X_train,parAWL); 
    Xrec = compose_signals(D_awli,code.A,code.Delta,nlatencies);
    D_awli = D_awli(nlatencies+next+1:end-nlatencies-next,:);
    for k=1:K
        D_awli(:,k)=D_awli(:,k)/norm(D_awli(:,k));
    end
    kernel_error(method_id,case_id,i) = kernel_dist_shift(D,D_awli);
...
[X,lats,amps] = create_examples(D_ext,t,parTrials);
sigma_X = sqrt(var(reshape(X,[1 size(X,1)*size(X,2)])));  
...
% zeropad trials 
l_atoms = size(X_train,1);
X_long = [zeros(nlatencies,nsignals); X_pink; zeros(nlatencies,nsignals)];
[~,D_motif2,A_motif2] = MoTIF_learning(X_long,l_atoms,K,1,0);
% normalize MoTIF atoms
for k=1:K
