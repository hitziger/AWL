...
/// create matrix for struct 
template <typename T>
inline T* createMatrixField(const char** field_names,int& field_count,
        const char* name, mxArray** fields, int rows, int cols) {
    field_names[field_count] = name;
    fields[field_count] = createMatrix<T>(rows,cols);
    return reinterpret_cast<T*>(mxGetPr(fields[field_count++]));
}

/// create struct 
inline mxArray* createStructMex(const char** field_names,const int field_count,
        mxArray** fields) {
    mxArray* out = mxCreateStructMatrix(1,1,field_count,field_names);
    const mwIndex zeroIndex=0;
    for (int i=0; i<field_count; ++i) {
        mxArray* test;
        test=fields[i];
        mxSetField(out,zeroIndex,field_names[i],fields[i]);
    }
    return out;
}
