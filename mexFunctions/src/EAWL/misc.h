...
static void sort(int* irOut, int beg, int end);
template <typename T>
...
static void sort(int* irOut,int beg, int end) {
   ...
   sort(irOut,beg,pivot-1);
   sort(irOut,pivot+1,end);
}
template <typename T>
static void quick_sort(int* irOut, T* prOut,const int beg, const int end, const bool incr) {
   if (end <= beg) return;
...
static void quick_sort(T1* irOut, T2* prOut,const int beg, const int end, const bool incr) {
   if (end <= beg) return;
   int pivot=beg;
   if (incr) {
