   ...
   public:
      ...
      void inline start() { _running=true;
         ...
         void inline reset() { _cumul=0;  
            ...
            void inline printElapsed(const char* msg= "Time elapsed");
            /// print the elapsed time
...
/// print the elapsed time
inline void Timer::printElapsed(const char* msg) {
   if (_running) {
      gettimeofday(_time2,NULL);
      mexPrintf("%s: %g seconds\n", msg, _cumul + static_cast<double>(
                  (_time2->tv_sec - _time1->tv_sec)*1000000
                  + _time2->tv_usec-_time1->tv_usec)/1000000.0);
   } else {
      mexPrintf("%s: %g seconds\n", msg, _cumul);
   }
};
